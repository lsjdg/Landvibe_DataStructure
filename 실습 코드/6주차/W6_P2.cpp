...
int main() {
     ...
     while (test--) {
         ...
         Node* leftNode = t.find(left);
         Node* rightNode = t.find(right);

         if (leftNode == nullptr || rightNode == nullptr)
             cout << "error\n";
         else {
             int result = (leftNode->depth - rightNode->depth > 0) ? leftNode->depth - rightNode->depth : (leftNode->depth - rightNode->depth) * -1;

             cout << leftNode->depth << ' ' << rightNode->depth << ' ' << result << '\n';
         }
     }
}